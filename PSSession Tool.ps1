...
if (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    $ps7Exists = Test-Path "C:\Program Files\PowerShell\7\pwsh.exe"
    if ($ps7Exists) {
        $msg = "Choose environment to run as Admin:`nYes - PowerShell ISE`nNo - PowerShell 7`nCancel - Standard PowerShell"
        $buttons = [System.Windows.Forms.MessageBoxButtons]::YesNoCancel
    } else {
        $msg = "Choose environment to run as Admin:`nYes - PowerShell ISE`nNo - Standard PowerShell`nCancel - Abort"
        $buttons = [System.Windows.Forms.MessageBoxButtons]::YesNoCancel
    }
    $msgBoxInput = [System.Windows.Forms.MessageBox]::Show($msg, "Choose Environment", $buttons, [System.Windows.Forms.MessageBoxIcon]::Question)
    if ($msgBoxInput -eq "Cancel" -and -not $ps7Exists) {
        exit
    }
    $exe = "powershell.exe"
    $args = "-NoProfile -File `"$($MyInvocation.MyCommand.Definition)`""
    if ($msgBoxInput -eq "Yes") {
        $exe = "powershell_ise.exe"
        $args = "-NoProfile `"$($MyInvocation.MyCommand.Definition)`""
    ...
    Start-Process $exe -Verb RunAs -ArgumentList $args
    exit
}
...
# Outer loop for restarting to initial choice
do {
    $restartToChoice = $true

    # Initial choice prompt
    $choice = [System.Windows.Forms.MessageBox]::Show("Choose:`nYes - Run Tool`nNo - Open PowerShell script", "Choice", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)
    if ($choice -eq "No") {
        # Provide option to select and run a .ps1 script as admin
        $runScriptChoice = [System.Windows.Forms.MessageBox]::Show("Do you want to select and run a .ps1 script as admin?", "Run Script", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)
        if ($runScriptChoice -eq "Yes") {
            $OpenFileDialog = New-Object System.Windows.Forms.OpenFileDialog
            $OpenFileDialog.Title = "Select PowerShell Script to Run as Admin"
            ...
            if ($Result -eq [System.Windows.Forms.DialogResult]::OK) {
                ...
                # Run the selected script as admin (since we're already elevated, it inherits)
                try {
                    & $scriptPath
                } catch {
        ...
        continue  # Go back to show initial choice again
    }

    # If choice is Yes, proceed to tool
    CLS
    ...
    try {
        ...
        continue  # Go back to initial choice on error
    }
    ...
    # Main script for tool
    try {
        Set-ExecutionPolicyIfNeeded
        if (-not $restartToChoice) { break }
