...
if (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    $msgBoxInput = [System.Windows.Forms.MessageBox]::Show("Choose environment to run as Admin:`nYes - PowerShell ISE`nNo - PowerShell 7`nCancel - Standard PowerShell", "Choose Environment", [System.Windows.Forms.MessageBoxButtons]::YesNoCancel, [System.Windows.Forms.MessageBoxIcon]::Question)
    $exe = "powershell.exe"
...
do {
    ...
    if ($choice -eq "No") {
        # Open the directory
        Start-Process explorer.exe -ArgumentList '\\brnas\pcsupport\Applications\Tech Support Tools\Powershell Scripts' -ErrorAction SilentlyContinue
        
        # Provide option to select and run a script as admin
        $runScriptChoice = [System.Windows.Forms.MessageBox]::Show("Directory opened. Do you want to select and run a .ps1 script as admin?", "Run Script", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)
